...
bool ObjParser::Load(char* filename)
{
	std::ifstream f;
	f.open(filename);
...
void ObjParser::ParseComment(std::ifstream& f)
{
	std::string str;
	std::getline(f, str);
}

void ObjParser::ParsePosition(std::ifstream& f)
{
	vec3 p;
...
void ObjParser::ParseTexCoord(std::ifstream& f)
{
	vec2 t;
...
void ObjParser::ParseNormal(std::ifstream& f)
{
	vec3 n;
...
void ObjParser::ParseFace(std::ifstream& f)
{
	std::string tri;
	std::string sub;
	int count = 0;
